...
<title>Lymphia &mdash; Logs</title>
...
.log-list{display:flex;flex-direction:column;gap:.4rem;max-width:800px}
.log-entry{background:rgba(255,255,255,.42);border:1px solid #d6d0c8;border-radius:8px;padding:.65rem 1rem;display:grid;grid-template-columns:9rem 4rem 1fr;align-items:baseline;gap:.75rem}
.log-ts{font-size:.72rem;font-weight:300;color:#9a9288;font-variant-numeric:tabular-nums;white-space:nowrap;letter-spacing:.02em}
...
.log-msg{font-size:.88rem;font-weight:300;color:#2d2a26;word-break:break-word;line-height:1.4}
.empty{color:#9a9288;font-size:.9rem;font-style:italic}
</style>
</head>
<body>
<header>
...
<hr>
<div id="logList" class="log-list"><p class="empty">Esperando entradas&hellip;</p></div>
<script>
function fmtTs(ms){
...
function render(data){
  ...
  if(!data||!data.length){el.innerHTML='<p class="empty">Sin entradas aún.</p>';return;}
  el.innerHTML=data.map(function(e){
    return'<div class="log-entry">'+
      '<span class="log-ts">'+fmtTs(e.ms)+'</span>'+
